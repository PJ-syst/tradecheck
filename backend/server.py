        ...
        def do_POST(self):
            ...
            try:
                ...
                if not 0 < length <= 8192:
                    raise InvalidRequest("Request body must be between 1 and 8192 bytes.")
                payload = json.loads(self.rfile.read(length))
                if not isinstance(payload, dict):
                    raise InvalidRequest("Request must be a JSON object.")
                path = urlparse(self.path).path
                if path == "/api/preview":
                    result = engine.preview(payload.get("message"))
...
def main():
    ...
    server = make_server(Engine(args.database, market=market, interpreter=interpreter, account=account), args.port)
